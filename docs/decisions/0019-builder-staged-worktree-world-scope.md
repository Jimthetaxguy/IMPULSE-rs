...
| Desktop launch wiring: registering with a staged scope, materializing before the PTY starts, and using `launch_working_directory` as the pane cwd | the desktop track (`impulse-desktop/**`) | Still open. The daemon now materializes during registration, so the desktop's remaining work is to declare the scope and take the pane cwd from `launch_working_directory()`. Promote/discard buttons are also still open; the desktop already presents the operator capability (#48). **Promote/discard buttons delivered 2026-09-12** (`claude/desktop-governed-controls-20260912`): the cockpit drives `PromoteGovernedOutcome` and `DiscardGovernedStagedWorktree` through `daemon_ops`'s acknowledged, operator-class request path, offers each control exactly when the shared `impulse-ops` predicate says the daemon would accept it, renders a blocked promotion as a typed per-reason banner with the canonical head and its remedy, and shows the unreferenced accepted commit's OID in a discard confirmation before sending. The same lane made the desktop's governed Git preflight **hook- and global-config-free** (`-c core.hooksPath=/dev/null -c core.fsmonitor=false` plus `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM=/dev/null`), closing the residual the P1 lane recorded against `run_bounded_governed_git`; because a repository-level `filter.*.clean` still executes during `git status` and **no Git switch disables it**, that preflight now refuses outright, by filesystem read, any workspace whose repository configuration defines an executable `filter`/`diff` driver. Claim, verification, and Supervisor review stay CLI-driven and Dioxus still renders their terminal command guidance, so `AGENTS.md` and `CLAUDE.md` now read "Dioxus exposes operator-only Promote and Discard controls (ADR-0019) and still shows terminal command guidance for claim, verify, and review, which remain CLI-driven" rather than the pre-ADR-0019 "rather than producer buttons". The staged *launch* half of this row — declaring the scope at registration and taking the pane cwd from `launch_working_directory()` — is still open. |
