...
| Decisions in GENOME.md | **Partially confirmed** | `GENOME.md` is a committed markdown file (`.impulse/GENOME.md`, per `CLAUDE.md`), but there is **no typed Rust read path** for it in `persistence.rs` — provenance of decisions is not yet structured per-agent. Insight-level provenance *does* exist: `impulse-ops/src/lib.rs:157` `InsightRecord { agent_label, kind, content, timestamp }`. |
...
| `Genome` | best-effort line parse of `GENOME.md` decisions tagged with an agent (gap today; degrade gracefully if untagged → `agent_id = "unknown"`) | decision text | `true` |
