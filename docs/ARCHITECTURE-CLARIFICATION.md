...
| Dioxus cockpit | `impulse-rs/impulse-desktop/src/{desktop_host,host_bridge,host_commands,daemon_sidecar,ui}.rs` | Renders one shared launch target, connected-daemon-project oversight/evidence, desktop-wide workers, and terminal state; dispatches acknowledged commands, packages the real host plus an exact-parent-watched daemon companion, and reaps only a desktop-owned daemon; does not own or optimistically mutate durable task truth | Packaged live host foundation; end-to-end role workflow hardening continues |
