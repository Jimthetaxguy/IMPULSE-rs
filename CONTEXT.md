...
**mediated**, never structural: nothing stops the process from writing outside the checkout. Since
protocol v9 the scope is reachable end to end: `RegisterGovernedTask` materializes the staged
worktree as part of registration (so the checkout exists before any PTY launch), and
`PromoteGovernedOutcome`/`DiscardGovernedStagedWorktree` are live endpoints — all three
operator-class, checked before any state read, with a blocked promotion answered as a *successful*
response carrying the typed outcome rather than an error.
