...
`CONFIRM` gate (`bash_exec` cwd is a write-root check; `/allow` does not widen it); a heuristic scan of `bash_exec`'s shell TEXT (absolute-path tokens -- including
...
ungated. `decide_step_model` is a different seam (which admitted model id this step uses) and is
not this confirmation list.
