...
pub fn sccache_setup(check_only: bool) -> Result<SccacheSetupResult> {
    ...
    let new_content = if existing.contains("[build]") {
        // Add rustc-wrapper under existing [build] section
        existing.replace("[build]", "[build]\nrustc-wrapper = \"sccache\"")
    } else {
        // Append new [build] section
    ...
    // Atomic write: temp file + rename to prevent corruption on interruption
...
fn is_sccache_configured(config_path: &PathBuf) -> bool {
...
mod tests {
    ...
    fn test_is_sccache_configured_without_entry() {
        ...
        assert!(!is_sccache_configured(&tmp.path().to_path_buf()));
    }
