...
echo "=== STEP4 GATE FULL + AC1 DESKTOP FEATURE ===" | tee "$SCRATCH/verify-step4.log"
(cd impulse-rs && cargo build --workspace && cargo check -p impulse-desktop --features desktop-app && cargo test --workspace && cargo clippy --workspace -- -D warnings && cargo fmt --check) 2>&1 | tee "$SCRATCH/verify-gate.log"
