...
- Use stable `agent_id`, `session_id`, and `owner_key` values from the provided snapshot.
- For targeted actions, populate the flat selector fields directly on the action object. Do not invent selector values.
