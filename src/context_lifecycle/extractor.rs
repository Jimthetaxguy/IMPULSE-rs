...
impl OutputExtractor {
    ...
    fn extract_remote_connection(line: &str) -> Option<String> {
        ...
        // smux launch or attach commands
        if trimmed.starts_with("smux ") || trimmed.contains(" smux ") {
            return Some(truncate_insight(trimmed, 120));
        }
        None
    }
...
mod tests {
    ...
    #[test]
    fn test_extract_remote_connection_detects_smux() {
        let insights = OutputExtractor::extract(
            AgentKind::ClaudeCode,
            1,
            "smux attach demo-shell --command 'bash -lc echo hi'",
        );
        assert!(insights
            .iter()
            .any(|insight| insight.insight_type == InsightType::RemoteConnection));
    }
