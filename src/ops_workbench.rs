...
fn overlay_agent_runtime(target: &mut AgentRuntime, overlay: &AgentRuntime) {
    ...
    target.agent_status = overlay.agent_status.clone();
    ...
    if overlay.ownership.is_some() {
        target.ownership = overlay.ownership.clone();
    }
    ...
    async fn test_build_snapshot_matches_session_id_before_agent_id() {
        ...
        let report = TerminalOpsReport {
            ...
            agents: vec![AgentRuntime {
                ...
                target: Some(impulse_ops::MachineTarget::Remote {
                    user: "james".to_string(),
                    host: "example.com".to_string(),
                    workdir: temp.path().display().to_string(),
                    session_name: Some("demo".to_string()),
                }),
                ownership: Some(impulse_ops::TerminalOwnership {
                    transport: impulse_ops::TerminalTransportKind::Tmux,
                    owner_key: "tmux:demo".to_string(),
                    workspace_key: "demo".to_string(),
                    session_name: Some("demo".to_string()),
                    note: Some("daemon-owned".to_string()),
                }),
                ..Default::default()
            }],
        ...
        assert_eq!(snapshot.agents[0].group.as_deref(), Some("tmux:demo"));
        assert_eq!(
            snapshot.agents[0]
                .ownership
                .as_ref()
        ...
        assert!(matches!(
            snapshot.agents[0].target.as_ref(),
            Some(impulse_ops::MachineTarget::Remote { host, .. }) if host == "example.com"
        ));
    }
