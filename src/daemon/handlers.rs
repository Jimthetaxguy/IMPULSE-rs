...
fn canonical_supervisor_target(
    agent: &impulse_ops::AgentRuntime,
) -> impulse_ops::SupervisorTargetRef {
    impulse_ops::SupervisorTargetRef {
        ...
        owner_key: agent
            .ownership
            .as_ref()
            .map(|ownership| ownership.owner_key.clone())
            .filter(|value| !value.is_empty())
            .or_else(|| agent.group.clone()),
    }
}

fn resolve_supervisor_target<'a>(
    snapshot: &'a impulse_ops::ProjectOpsSnapshot,
    target: &impulse_ops::SupervisorTargetRef,
) -> Result<&'a impulse_ops::AgentRuntime> {
    if let Some(session_id) = target.session_id() {
        return snapshot
            .agents
            .iter()
            .find(|agent| agent.session_id.as_deref() == Some(session_id))
            .ok_or_else(|| anyhow::anyhow!("No agent matched session_id '{}'", session_id));
    }

    if let Some(owner_key) = target.owner_key() {
        let matches = snapshot
            .agents
            .iter()
            .filter(|agent| {
                agent.group.as_deref() == Some(owner_key)
                    || agent
                        .ownership
                        .as_ref()
                        .map(|ownership| ownership.owner_key.as_str())
                        == Some(owner_key)
            })
            .collect::<Vec<_>>();
        ...
        if matches.len() > 1 {
            ...
            let labels = matches
                .iter()
                .map(|agent| format!("{} ({})", agent.label, agent.id))
                .collect::<Vec<_>>()
                .join(", ");
            anyhow::bail!(
                "owner_key '{}' matched multiple agents: {}",
                owner_key,
                labels
            );
        }
    ...
    if let Some(agent_id) = target.agent_id() {
        return snapshot
            ...
            .ok_or_else(|| anyhow::anyhow!("No agent matched agent_id '{}'", agent_id));
    }

    anyhow::bail!("Supervisor action target is missing agent_id, session_id, and owner_key")
}
...
async fn run_supervisor_action(
    ...
    match action.clone() {
        impulse_ops::SupervisorAction::FocusAgent { target } => {
            let resolved = resolve_supervisor_target(&snapshot, &target)?;
            Ok(impulse_ops::SupervisorActionResult {
                status: "dispatch_local".to_string(),
                message: format!("Approved focus for {}", resolved.label),
                local_action: Some(impulse_ops::SupervisorAction::FocusAgent {
                    target: canonical_supervisor_target(resolved),
                }),
        ...
        impulse_ops::SupervisorAction::SendInput {
            ...
            Ok(impulse_ops::SupervisorActionResult {
                status: "dispatch_local".to_string(),
                message: format!("Approved input dispatch for {}", resolved.label),
                local_action: Some(impulse_ops::SupervisorAction::SendInput {
                    target: canonical_supervisor_target(resolved),
                    content,
                    confirmed,
                }),
        ...
        impulse_ops::SupervisorAction::InjectContext { target, query, .. } => {
            let target = if target.is_empty() {
                None
            } else {
                Some(resolve_supervisor_target(&snapshot, &target)?)
            };
        ...
        impulse_ops::SupervisorAction::CleanupContext { target, goal, .. } => {
            let target = resolve_supervisor_target(&snapshot, &target)?;
...
mod tests {
    use super::*;
    use impulse_ops::{
        AgentRuntime, ContextHealthSummary, SupervisorAction, SupervisorPermissionPolicy,
        SupervisorTargetRef, TerminalOpsReport, TerminalOwnership, TerminalTransportKind,
    };
    use tempfile::TempDir;
    use tokio::sync::RwLock;

    fn ownership(owner_key: &str) -> TerminalOwnership {
        TerminalOwnership {
            transport: TerminalTransportKind::Tmux,
            owner_key: owner_key.to_string(),
            workspace_key: "demo".to_string(),
            session_name: Some("demo".to_string()),
            note: Some("mux".to_string()),
        }
    }

    fn telemetry_report(source_id: &str, agents: Vec<AgentRuntime>) -> TerminalOpsReport {
        TerminalOpsReport {
    ...
    #[tokio::test]
    async fn test_run_supervisor_action_focus_agent_prefers_session_id() {
        ...
        let session = shared
            ...
            .await
            .unwrap();
        let project_id = crate::ops_workbench::project_summary(&shared).id;
        let mut store = crate::ops_workbench::TerminalOpsTelemetryStore::default();
        store.publish(
            &project_id,
            telemetry_report(
                "gui-test",
                vec![AgentRuntime {
                    id: "tab-99".to_string(),
                    label: "Codex: demo".to_string(),
                    backend_kind: "Codex".to_string(),
                    session_id: Some(session.id.clone()),
                    active: true,
                    group: Some("tmux:demo".to_string()),
                    ownership: Some(ownership("tmux:demo")),
                    ..Default::default()
                }],
            ),
        );
        let telemetry = std::sync::Arc::new(RwLock::new(store));
        let session_override = std::sync::Arc::new(RwLock::new(None::<SupervisorPermissionPolicy>));

        let result = run_supervisor_action(
            &shared,
            SupervisorAction::FocusAgent {
                target: SupervisorTargetRef {
                    agent_id: Some("wrong-agent".to_string()),
                    session_id: Some(session.id.clone()),
                    owner_key: Some("tmux:demo".to_string()),
                },
            },
            &telemetry,
            &session_override,
        ...
        assert_eq!(result.status, "dispatch_local");
        match result.local_action {
            Some(SupervisorAction::FocusAgent { target }) => {
                assert_eq!(target.session_id.as_deref(), Some(session.id.as_str()));
                assert_eq!(target.owner_key.as_deref(), Some("tmux:demo"));
            }
            other => panic!("expected focus local action, got {:?}", other),
        }
    }

    #[tokio::test]
    async fn test_run_supervisor_action_focus_agent_rejects_ambiguous_owner_key() {
        let temp = TempDir::new().unwrap();
        let shared =
            std::sync::Arc::new(crate::state::State::new(temp.path().to_path_buf()).unwrap());
        let project_id = crate::ops_workbench::project_summary(&shared).id;
        ...
        store.publish(
            ...
            telemetry_report(
                ...
                vec![
                    AgentRuntime {
                        id: "tab-1".to_string(),
                        label: "Codex: one".to_string(),
                        backend_kind: "Codex".to_string(),
                        active: true,
                        group: Some("tmux:shared".to_string()),
                        ownership: Some(ownership("tmux:shared")),
                        ..Default::default()
                    },
                    AgentRuntime {
                        id: "tab-2".to_string(),
                        label: "Codex: two".to_string(),
                        backend_kind: "Codex".to_string(),
                        active: true,
                        group: Some("tmux:shared".to_string()),
                        ownership: Some(ownership("tmux:shared")),
        ...
        let err = run_supervisor_action(
            &shared,
            SupervisorAction::FocusAgent {
                target: SupervisorTargetRef {
                    agent_id: None,
                    session_id: None,
                    owner_key: Some("tmux:shared".to_string()),
                },
            },
            &telemetry,
            &session_override,
        ...
        assert!(format!("{err:#}").contains("matched multiple agents"));
    }
}
