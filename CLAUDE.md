...
**Current canonical projection (2026-07-12):** 1,895 passed, 8 ignored, 0 failed. The verified isolated agent-cache-serialization worktree reports 1,894 passed with the canonical-checkout-only archive proof explicitly filtered.
...
```
Expected canonical checkout projection: 1,895 passed across the 5 crates (impulse-rs, impulse-ops, impulse-term, impulse-desktop, impulse-ion), with 8 ignored and 0 failed. The verified isolated worktree without the gitignored reconciliation archive reports 1,894 passed when that one proof is explicitly filtered. Re-run the full gate before treating changed counts as verified; if they change, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,895 canonical workspace projection expected, 8 ignored; verify with `cargo test --workspace 2>&1 | grep "test result:"`)
