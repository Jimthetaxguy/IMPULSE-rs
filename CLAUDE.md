...
```
Expected: 1,846 passed across the 5 crates (impulse-rs, impulse-ops, impulse-term, impulse-desktop, impulse-ion). If this changes, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,846 workspace total expected, 7 ignored; verify with `cargo test --workspace 2>&1 | grep "test result:"`)
