...
Impulse can strongly control launch conditions, process lifecycle, exposed tools, credentials, filesystem boundaries, and observable policy gates. It cannot replace hidden prompts, proprietary reasoning loops, or unsupported internals of third-party CLIs.
...
The live code has a supervisor-specific `SupervisorPermissionPolicy` and a narrow coordinator/worker `AgentRole`; it does **not** yet have the generalized role contract above. Likewise, the pending aggregate makes desktop launch identity registry-driven, but a common runtime-adapter trait and capability-negotiation protocol remain future ADR work. Do not describe all runtimes as structurally equivalent or fully governed.
...
**Current canonical expectation (2026-07-11):** 1,892 passed, 7 ignored, 0 failed. The isolated agent-cache-serialization worktree is expected to report 1,891 passed with the canonical-checkout-only archive proof explicitly filtered; both counts must be re-verified after this rebase.

**Historical verification narrative:**
- `cargo test --workspace`: 1,882 passed, 7 ignored, 0 failed (verified 2026-07-11; impulse-rs unittests alone: 1,485 + guardrail-confirmation follow-up, see below. T7 fixes (Opus adversarial review of commit bf38b06) landed in `bash_exec.rs` — `Command::kill_on_drop(true)` so a timed-out child is actually killed instead of orphaned (matches the earlier `pi_adapter.rs` timeout-kill fix), and a `truncate_at_char_boundary` helper replacing a raw `String::truncate` that could panic mid-multi-byte-char on >256KiB output — plus `tool_verify.rs`'s `ok` now mirrors the CLI's `!passed() || validate().is_err()` logic exactly (was `passed()`-only, missing `MissingCommandsRun`-class violations) and the JSON payload gained the `contract_violation` field the CLI's envelope already had. T8 added `src/ion_repl/chat.rs` (`ChatState` wrapping `llm_backends::Agent` as the REPL's chat session — free-text lines now reach a real LLM turn, `/clear` truly clears history, missing `ANTHROPIC_API_KEY` degrades to a one-line notice instead of a panic). T9 (final REPL-roadmap item) added tool-calling: `llm_backends::mod.rs` gained `ToolDefinition`/`ToolCall`/`ToolResult`/`StopReason`/`ToolExecutor`/`ToolExecutionResult` plus `Agent::chat_with_tools`/`chat_with_tools_capped` (a request → tool_use → execute → tool_result loop capped at `DEFAULT_MAX_TOOL_ROUNDS = 10`, erroring with the new `AgentError::ToolLoopLimitExceeded` variant instead of looping forever); `AnthropicProvider::chat` (`llm_backends/anthropic.rs`) sends `ChatRequest::tools` and parses `tool_use` blocks + `stop_reason` via a new `format_anthropic_messages` helper; `ChatState::turn` gained `&ReplToolRegistry`/`&ReplContext` params and a `ReplToolExecutor` adapter so every chat turn can trigger `ion_verify`/`file_write`/`bash_exec` conversationally. Opus's adversarial review of T9 found a real safety gap (finding S1): `bash_exec`/`file_write` became reachable from raw model output for the first time with no confirmation step — fixed same-day by gating `CONFIRMATION_REQUIRED_TOOLS` behind a `confirm` hook in `ReplToolExecutor` (`confirm_via_stdin` in production, default-deny; `ChatState::with_confirm` test seam), with regression tests proving a decline is a true short-circuit (the shell command physically never runs). Net vs. T8's 1,825 baseline: +21 (T9: +18, confirmation-gate tests: +3). Note: `impulse-term::test_with_parser_reads_screen_size` and 3 impulse-desktop tests (`test_dioxus_desktop_launch_binary_is_feature_gated`, `test_host_readiness_smoke_script_is_declared`, `test_xterm_vendor_assets_are_present_and_manifested`) are parallelism-flaky under full `--workspace` load — pass in isolation/serial per-crate. Also flaky under `--workspace`: `handlers::common::tests::test_persist_claude_env_var_creates_file_with_assignment` races a sibling test over the shared `CLAUDE_ENV_FILE` env var (no lock) — pre-existing, unrelated to ion work, passes on isolated/serial rerun. **Env-scrubbing follow-up (same day):** `bash_exec.rs` gained `ENV_ALLOWLIST`/`is_secret_like` plus the `.env_clear()` + allowlist-re-add call in `execute()` (see Architecture section); net +4 tests (`test_is_secret_like_matches_known_credential_shapes`, `test_is_secret_like_does_not_flag_allowlisted_names`, `test_execute_scrubs_secret_env_vars_from_child_process`, `test_execute_still_has_path_and_can_run_ordinary_commands`). Net vs. the confirmation-gate's 1,846 baseline: +4 = 1,850. **Env-scrubbing extended to `ProcessTool` (same day, follow-up audit):** an audit of the ~40 `Command::new` call sites across `src/` and `impulse-ion/src/` for the same secret-leak shape found `src/tooling/external.rs`'s `ProcessTool::execute` (manifest-defined external tools, e.g. `.impulse/tools.d/*.json`, loaded via `ToolRegistry::with_runtime()`) had the identical bug — `tokio::process::Command` inheriting the full parent env, with the manifest's own `env_allowlist` field being redundantly re-added on top of, not instead of, that full inheritance. `bash_exec.rs`'s `ENV_ALLOWLIST`/`is_secret_like`/scrub logic was extracted into a new shared `src/tooling/env_scrub.rs` module (`scrub_and_allowlist_env(cmd, extra_allowlist)`) so both call sites share one implementation; `external.rs`'s `execute()` now calls it with the manifest's `env_allowlist` as the extra, per-tool allowlist. `impulse-ion/src/pi_adapter.rs`'s `bash` launcher subprocess was investigated and left unchanged — it's a developer-configured launcher script (`ION_GATE_LAUNCHER` env var, defaults to a promoted `~/.ai-memory` path), not itself running LLM-generated shell text, and it plausibly needs full env access to authenticate against MiniMax (its own API keys); blanket `.env_clear()` there would risk a functional regression with no corresponding LLM-reachable secret-leak vector today (`ion_repl`'s only LLM tool-calling loop uses `ToolRegistry::with_defaults()`, which excludes `ProcessTool` entirely — confirmed no code path currently lets a `ProcessTool` run with LLM-originated arguments; the `external.rs` fix is defense-in-depth for the daemon `InvokeTool`/`tooling-run` human-driven paths, ahead of `ProcessTool` potentially being bridged into `ion_repl` later). Net vs. the 1,850 baseline: +5 (env_scrub.rs gained 4 tests including the 2 relocated `is_secret_like` tests moved out of `bash_exec.rs`, net +2 there; `external.rs` gained 3 new tests: `test_execute_scrubs_secret_env_vars_from_child_process`, `test_execute_forwards_explicitly_allowlisted_manifest_vars`, `test_execute_still_has_path_after_scrub`) = 1,855. **Wall-clock timeout follow-up (same day, S2):** net +2 (`test_agent_error_tool_loop_timed_out_display` in `error.rs`, `test_chat_with_tools_capped_timeout_returns_error_when_timeout_hit` in `llm_backends/mod.rs`) = 1,857. **Fresh-eyes sweep follow-up (same day, finding G2):** `ion_repl::tool_bridge::DynamicToolBridge::run` hardcoded `ok: true` for every successfully-*executed* tool call regardless of the tool's own logical result -- a `bash_exec` command exiting non-zero (an `Ok` result with `"success": false` in its JSON payload, not an `Err`) was reported to the model as `is_error: false`. Fixed by deriving `ok` from `output["success"]` when present, defaulting to `true` for tools with no such field (file_read/file_write, unchanged). Net +2 (`run_reports_ok_false_when_the_tools_own_payload_declares_failure`, `run_reports_ok_true_for_a_tool_with_no_success_field_in_its_payload`) = 1,859. The same sweep's finding G1 (the wall-clock timeout doesn't bound a turn parked at the `confirm_via_stdin` blocking prompt, since a synchronous `stdin().read_line()` isn't an `.await` point `tokio::time::timeout` can cancel) was accepted as a documented limitation rather than fixed -- `ChatState::turn`'s doc comment now states it explicitly; forcing the read through `spawn_blocking` wouldn't make it cancellable either and risks a stray stdin read racing a later prompt.) **Guardrail-scanned confirmation follow-up (same day, ROSA reverse-transfer):** net +10 tests in `src/ion_repl/chat.rs` (`test_decide_approval_block_tier_rejects_plain_yes_and_requires_literal_confirm`, `test_decide_approval_warn_and_no_match_tiers_accept_plain_yes`, `test_block_tier_bash_exec_is_not_approved_by_a_confirm_stub_that_would_say_yes`, `test_guard_verdict_for_bash_exec_scans_command_against_bash_target`, `test_guard_verdict_for_warn_tier_command_surfaces_reason_and_rule_id`, `test_guard_verdict_for_benign_command_yields_no_verdict`, `test_guard_verdict_for_file_write_scans_content_against_filewrite_target_not_bash`, `test_file_write_matching_block_tier_rule_is_not_approved_by_plain_yes`, `test_approval_grant_is_only_minted_after_a_genuine_true_from_confirm`, `test_approval_grant_carries_the_tool_name_and_guard_action_it_was_minted_with`) = 1,869. The 3 existing S1-regression tests (`test_bash_exec_is_gated_and_a_decline_prevents_the_command_running`, `test_read_only_tools_are_not_gated_behind_confirmation`, `test_turn_with_confirm_declined_does_not_execute_bash_exec`) were updated for the new 3-arg `confirm` signature with no assertion changes. **`block-write-secret` built-in rule follow-up (same day):** the guardrail-confirmation feature above shipped with `file_write`'s scan correctly wired to `GuardTarget::FileWrite` but no built-in rule actually targeting that surface (`guardrail::defaults::builtin_rules()` had 9 rules, all `GuardTarget::Bash`) — dormant, not broken. Added a 10th built-in rule, `block-write-secret` (`src/guardrail/defaults.rs`, ported from a sibling project's equivalent pattern, itself originally from an earlier version of this module): matches `key_name = "value"`/`key_name: value` shapes for `api_key`/`secret`/`token`/`password`-ish names with a 16+ char value, targeting `GuardTarget::FileWrite`. This makes `file_write`'s Block-tier `CONFIRM`-required escalation actually fire against real defaults for the first time. Updated the two `ion_repl/chat.rs` tests that previously had to construct an equivalent custom `GuardRule` (since no built-in existed) to use `GuardConfig::default()` directly against the new real rule. Net +1 test (`test_blocks_writing_a_hardcoded_secret_to_a_file` in `guardrail/defaults.rs`; the existing `test_default_rules_all_enabled` count assertion updated 9→10) = 1,870. **Daemon agent-IPC freeze fix + bounded reads follow-up (same-day, outside `ion_repl` — see Architecture section):** net +8 tests — `agent/mod.rs` gained `test_harness_query_times_out_instead_of_hanging_forever` and `test_harness_query_kills_hung_child_instead_of_orphaning`; `error.rs` gained `test_agent_error_harness_timed_out_display`; `daemon/mod.rs` gained a `bounded_line_tests` module with 4 tests (`test_read_bounded_line_reads_lines_under_cap_normally`, `test_read_bounded_line_accepts_a_line_exactly_at_the_cap`, `test_read_bounded_line_rejects_a_line_one_byte_over_the_cap`, `test_read_bounded_line_rejects_oversized_input_without_unbounded_memory_use`); `daemon/handlers.rs` gained `test_checkout_agent_releases_lock_before_a_slow_query_completes` (`agent_lock_release_tests` module). = 1,878. **Harness-timeout process-group fix (same day, teammate-caught regression in the daemon fix above):** a teammate Claude session's independent re-verification of the daemon fix found `test_harness_query_kills_hung_child_instead_of_orphaning` deterministically failing (not flaky, reproduced 5x) -- `kill_on_drop` only SIGKILLs the *direct* child, and the test's fake harness is a shell script, so the kernel execs `/bin/sh` to run it, which then forks the actual hung work as its own grandchild rather than exec-replacing itself. Fixed with the same `process_group(0)`-at-spawn + explicit `kill -KILL -- -<pgid>`-on-timeout pattern `impulse_ion::pi_adapter`'s watchdog already used earlier the same day for an identical problem; verified 15/15 consecutive clean runs. **Same fix applied to `bash_exec.rs`** (same day, follow-up): confirmed live (`sh -c "sleep 30 & wait"`, kill the `sh` pid, the `sleep` pid survives) that `bash_exec`'s `sh -c "<command>"` invocation has the identical latent gap for any compound (`cmd1 && cmd2`) or explicitly backgrounded (`cmd &`) command -- `sh` only exec-replaces itself for a single simple command, so the existing kill-on-timeout test happened to pass only because it used a bare `sleep N`. Since `bash_exec` runs arbitrary LLM-generated commands (the most-hardened surface this session), this was a real, currently-exploitable orphan-process gap, not hypothetical. Same `process_group(0)`/group-kill fix applied; new regression test `test_execute_kills_grandchild_of_a_backgrounded_compound_command_on_timeout` uses `sleep N & wait` specifically to exercise the fork (not exec-replace) path the existing test didn't cover. Net +1 test = 1,879. **Keychain secret-argv fix (same day, Opus sweep finding):** net +1 test (`test_set_get_delete_round_trips_via_stdin_prompt_not_argv` in `credentials/keychain.rs`, macOS-gated) = 1,880. **Test-marker collision fix (same day, Fable review finding):** no test-count change (3 existing tests' marker formula edited in place, no tests added/removed). **pgrep-availability vacuous-pass fix (same day, Fable review finding):** the same 3 orphan-kill regression tests (`agent/mod.rs`'s `test_harness_query_kills_hung_child_instead_of_orphaning`, `bash_exec.rs`'s `test_execute_kills_child_on_timeout_instead_of_orphaning` and `test_execute_kills_grandchild_of_a_backgrounded_compound_command_on_timeout`) wrapped their post-kill `pgrep` verification in `if let Ok(check) = ...` — if `pgrep` failed to spawn at all (not installed / not on PATH), the branch was silently skipped and the test passed without checking anything. Replaced with `.expect("pgrep unavailable -- ...")` so a missing `pgrep` binary is now a hard test failure instead of a silent pass; a successful `pgrep` run that simply finds no matching pids (its normal exit-code-1 "no match" case) is unaffected and still asserts cleanly. No test-count change (3 existing tests edited in place). These 3 tests plus `handlers::common::tests::test_persist_claude_env_var_creates_file_with_assignment` remain documented parallelism-flaky under full `--workspace --test-threads` load (all 4 reconfirmed clean when re-run in isolation with `--test-threads=1` immediately after this fix, ruling out a regression). **`file_read` overflow + sandbox-bypass fix (same day, second Opus fresh-eyes sweep):** `src/tooling/builtin/file_read.rs` was never touched by any of the day's other hardening -- an independent fresh-eyes sweep found two real gaps unique to it. (1) `start_line`/`max_lines` come straight from model tool-call JSON with no upper bound; `start_line - 1 + max_lines` could overflow `usize` on a near-`u64::MAX` `start_line` -- a debug-build panic, or in release a wraparound that produces a bogus small `end` and then a slice-index-out-of-range panic once `total_lines` is known -- either way an uncaught panic (no `catch_unwind` around `ReplToolExecutor::execute`) that would kill the whole `ion` session on a single malicious/malformed tool call. Fixed with saturating arithmetic (`start_line.saturating_sub(1).min(total_lines)`, `start.saturating_add(max_lines).min(total_lines)`) instead of raw subtraction/addition. (2) unlike `file_write` (which calls `ctx.is_path_allowed(&path, true)`), `file_read`'s `execute` took `_ctx: &ToolContext` and never consulted it at all -- no sandboxing on a read-only tool that's also *ungated* by the confirmation gate (only `bash_exec`/`file_write` are in `CONFIRMATION_REQUIRED_TOOLS`), so a model or prompt-injected content could ask to read e.g. `~/.ssh/id_rsa` and have its contents streamed straight into the model's own context with zero friction. Fixed by resolving the path via `ctx.resolve_path` and gating on `ctx.is_path_allowed(&path, false)`, matching `file_write`'s existing pattern exactly. **Scope caveat:** this closes the wiring gap (the tool now respects `ToolContext`'s read-root allowlist when one is configured), but does not by itself restrict anything in the live `ion_repl` path today -- `tool_bridge.rs`'s own doc comment already documents that the REPL currently runs every adapted tool under `ToolContext::with_all_capabilities()` (unrestricted roots), with a narrower per-session `ToolContext` explicitly called out as a "natural T9+ follow-up." This fix makes `file_read` consistent with `file_write` and ready for that follow-up, but the follow-up itself remains open. Net +2 tests (`test_execute_does_not_panic_or_overflow_on_near_u64_max_start_line`, `test_execute_denies_a_path_outside_allowed_read_roots`) = 1,882.
- **Managed-agent lifecycle follow-up (2026-07-11):** Replaced daemon `checkout_agent`/`checkin_agent` ownership transfer with one in-place Tokio mutex guard for each bounded logical turn. Concurrent turns now fail fast through protocol-v3 typed Busy, removing double initialization, ghost queued turns, last-writer-wins loss of conversation/recommendation/pane-summary state, and cache loss when a handler future is cancelled. A shared RAII process-group guard also kills harness and `bash_exec` grandchildren on timeout or arbitrary task abort, and every fallback HTTP request carries a local 120-second bound. Eleven focused tests (ten net over the replaced lock-release test) cover Busy wire/client semantics, ordered retry history, abort recovery, provider bounds, process cleanup, and unrelated daemon responsiveness. Full canonical expectation is now 1,889.
...
```
Expected canonical checkout after the live-branch rebase: 1,892 passed across the 5 crates (impulse-rs, impulse-ops, impulse-term, impulse-desktop, impulse-ion), with 7 ignored and 0 failed. An isolated worktree without the gitignored reconciliation archive is expected to report 1,891 passed when that one proof is explicitly filtered. Re-run the full gate before treating these counts as verified; if they change, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,892 canonical workspace total expected, 7 ignored; verify with `cargo test --workspace 2>&1 | grep "test result:"`)
