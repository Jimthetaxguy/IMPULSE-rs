...
```
Expected canonical checkout: 1,931 passed across the 5 crates (impulse-rs, impulse-ops, impulse-term, impulse-desktop, impulse-ion), with 8 ignored and 0 failed. An isolated worktree without the gitignored reconciliation archive reports 1,930 passed when that one proof is explicitly filtered. If this changes, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,931 canonical workspace total expected, 8 ignored; verify with `cargo test --workspace 2>&1 | grep "test result:"`)
