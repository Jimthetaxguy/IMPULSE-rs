...
- `cargo test --workspace`: 1,371 unit + 26 integration (impulse-rs) + 31 (impulse-ops) + 114 (impulse-term) + 159 (impulse-desktop) = 1,701 passed, 4 ignored, 0 failed (verified 2026-06-26)
...
```
Expected: 1,701 passed across the 4 crates (impulse-rs, impulse-ops, impulse-term, impulse-desktop). If this changes, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,701 workspace total expected: 1371+26 impulse-rs, 31 ops, 114 term, 159 desktop; verify with `cargo test --workspace 2>&1 | grep "test result:"`)
