...
**Workspace (4 crates):**
- `impulse-rs/` — main CLI + daemon + TUI (64,068 LOC in src/, 1,317 unit + 26 integration tests across 178 .rs files)
...
- `cargo test`: 5 `test result:` lines totaling 1,343 passed, 3 ignored, 0 failed (plus 240 GUI, 110 term when run per-crate; 1,697 total)
...
```
Expected: 1,343 passed. If this changes, update both this section and the Architecture section.

### Pre-Commit Checklist

1. `cargo build` — zero warnings
2. `cargo test` — all tests pass (1,343 workspace total expected: 1317+26 impulse-rs, 4 ops, 110 term, 240 gui; verify with `cargo test 2>&1 | grep "test result:"`)
