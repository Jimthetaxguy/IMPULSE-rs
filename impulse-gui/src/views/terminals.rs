...
pub(super) struct Tab {
    ...
    pub(super) ownership: TerminalOwnership,
    pub(super) launch_warning: Option<LaunchFallbackWarning>,
}
...
impl TerminalsView {
    ...
    pub fn spawn_tab(&mut self, agent: &AgentInfo, target_dir: &Path, _ctx: &egui::Context) {
        ...
        let id = self.next_id;
        self.next_id += 1;
        let launch_plan = build_launch_plan(agent.name, agent.command, agent.args, target_dir);
        ...
        match TerminalPanel::spawn_with_theme(
            ...
            Ok(panel) => {
                ...
                let tab = Tab {
                    ...
                    ownership: launch_plan.ownership,
                    launch_warning: launch_plan.warning.clone(),
                };
                ...
                let delay = Duration::from_millis(
                    AgentKind::detect(agent.command, agent.name).startup_delay_ms(),
                );
                ...
                if let Some(warning) = launch_plan.warning {
                    log::warn!("Terminal launch fallback for tab {}: {}", id, warning);
                }
            }
            Err(e) => {
    ...
    fn resolve_tab_id_for_target(&self, target: &SupervisorTargetRef) -> Option<u64> {
        if let Some(session_id) = target.session_id() {
            if let Some((tab_id, _)) = self.tabs.iter().find(|(tab_id, tab)| {
                let runtime_id = format!("tab-{}", tab_id);
                runtime_id == session_id || tab.daemon_session_id.as_deref() == Some(session_id)
            }) {
                return Some(*tab_id);
            }
        }
        ...
        if let Some(owner_key) = target.owner_key() {
            let matches = self
                .tabs
                .iter()
                .filter(|(_, tab)| tab.ownership.owner_key == owner_key)
                .collect::<Vec<_>>();
            if matches.len() == 1 {
            ...
            if matches.len() > 1 {
                if let Some(agent_id) = target.agent_id() {
                    let narrowed = matches
                        .into_iter()
                        .filter(|(tab_id, tab)| {
                            format!("tab-{}", tab_id) == agent_id
                                || tab.label == agent_id
                                || tab.agent_name == agent_id
                        })
                        .collect::<Vec<_>>();
                    if narrowed.len() == 1 {
                        return Some(*narrowed[0].0);
                    }
                }
                return None;
            }
        ...
        let matches = self
            .tabs
            .iter()
            .filter(|(_, tab)| tab.label == agent_id || tab.agent_name == agent_id)
            .collect::<Vec<_>>();
        if matches.len() == 1 {
            Some(*matches[0].0)
        } else {
    ...
    pub fn focus_agent(&mut self, target: &SupervisorTargetRef) -> bool {
        self.resolve_tab_id_for_target(target)
            .map(|tab_id| self.focus_tab(tab_id))
            .unwrap_or(false)
    }

    pub fn send_to_agent(&self, target: &SupervisorTargetRef, content: &str) -> bool {
        self.resolve_tab_id_for_target(target)
                            ...
                            for agent in &snapshot.agents {
                                ui.group(|ui| {
                                    ...
                                    ui.horizontal_wrapped(|ui| {
                                        ...
                                        if let Some(ownership) = &agent.ownership {
                                            ui.separator();
                                            let transport = match ownership.transport {
                                                TerminalTransportKind::Direct => "direct",
                                                TerminalTransportKind::Tmux => "tmux",
                                                TerminalTransportKind::Smux => "smux",
                                            };
                                            let transport_label = ownership
                                                .session_name
                                                .as_ref()
                                                .map(|session| format!("{}:{}", transport, session))
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    for id in tab_ids {
                        if let Some(tab) = self.tabs.get(&id) {
                            ...
                            let label = match tab.ownership.transport {
                                TerminalTransportKind::Direct => format!("{} #{}", tab.label, id),
                                _ => format!(
                                    "{} #{} [{}]",
                                    tab.label,
                                    id,
                                    tab.ownership.session_name.as_deref().unwrap_or({
                                        match tab.ownership.transport {
                                            TerminalTransportKind::Tmux => "tmux",
                                            TerminalTransportKind::Smux => "smux",
                                            TerminalTransportKind::Direct => "direct",
                                        }
...
mod tests {
    ...
    fn spawn_test_tab(
        ...
        Tab {
            ...
            daemon_session_id: daemon_session_id.map(str::to_string),
            created_at: Instant::now(),
    ...
    fn shutdown_tabs(view: &mut TerminalsView) {
    ...
    #[test]
    fn test_resolve_tab_id_for_target_prefers_session_id() {
        let mut view = TerminalsView::new(None);
        view.tabs.insert(
            1,
            spawn_test_tab(1, "Codex: one", "Codex", Some("sess-one"), "tmux:alpha"),
        );
        view.tabs.insert(
            2,
            spawn_test_tab(2, "Codex: two", "Codex", Some("sess-two"), "tmux:alpha"),
        );

        let resolved = view.resolve_tab_id_for_target(&test_target(
            Some("tab-2"),
            Some("sess-one"),
            Some("tmux:alpha"),
        ));
        ...
        shutdown_tabs(&mut view);
    }

    #[test]
    fn test_resolve_tab_id_for_target_rejects_ambiguous_owner_key_without_agent_hint() {
        let mut view = TerminalsView::new(None);
        view.tabs.insert(
            1,
            spawn_test_tab(1, "Codex: one", "Codex", Some("sess-one"), "tmux:shared"),
        );
        view.tabs.insert(
            2,
            spawn_test_tab(2, "Codex: two", "Codex", Some("sess-two"), "tmux:shared"),
        );

        let resolved =
            view.resolve_tab_id_for_target(&test_target(None, None, Some("tmux:shared")));
    ...
    #[test]
    fn test_focus_and_send_to_agent_use_target_resolution() {
        let mut view = TerminalsView::new(None);
        view.tabs.insert(
            7,
            spawn_test_tab(7, "Codex: demo", "Codex", Some("sess-seven"), "tmux:demo"),
        );

        let target = test_target(Some("tab-7"), Some("sess-seven"), Some("tmux:demo"));
        assert!(view.focus_agent(&target));
        assert_eq!(view.active_tab, Some(7));
        assert!(view.send_to_agent(&target, "echo hi\n"));
