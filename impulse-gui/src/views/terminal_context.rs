    ...
    pub fn workbench_agents(&self) -> Vec<impulse_ops::AgentRuntime> {
        self.tabs
            ...
            .map(|(id, tab)| {
                ...
                let remote_connections = tab.panel.recent_remote_connections(8);
                let ownership = infer_ownership(
                    &tab.ownership,
                    remote_connections.iter().map(|s| s.as_str()),
                );
                ...
                if let Some(warning) = &tab.launch_warning {
                    warnings.push(warning.to_string());
                }
                if ownership.transport != TerminalTransportKind::Direct {
                    warnings.push(format!(
                        "Terminal ownership routed through {} ({})",
                        match ownership.transport {
                ...
                impulse_ops::AgentRuntime {
                    ...
                    session_id: tab.daemon_session_id.clone(),
                    ephemeral: true,
                    ...
                    role: None,
                    group: Some(ownership.owner_key.clone()),
            ...
            if insights.len() > snapshot.insight_count {
                ...
                for insight in new_insights {
                    match insight.insight_type {
                        ...
                        InsightType::DecisionMade | InsightType::RemoteConnection => {}
                    }
                }
