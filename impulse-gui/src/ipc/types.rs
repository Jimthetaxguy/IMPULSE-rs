    ...
    fn test_daemon_request_run_supervisor_action_roundtrip() {
        ...
        assert!(matches!(
            ...
            DaemonRequest::RunSupervisorAction {
                action: impulse_ops::SupervisorAction::FocusAgent { target }
            } if target.agent_id.as_deref() == Some("agent-1")
                && target.session_id.as_deref() == Some("sess-1")
                && target.owner_key.as_deref() == Some("tmux:demo")
        ));
    }
