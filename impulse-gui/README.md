...
The active GUI runtime now treats terminal transport as explicit metadata instead of assuming every tab is a direct PTY.

- Default: direct PTY ownership per tab
- `IMPULSE_TERMINAL_TRANSPORT=tmux`: wrap new tabs in `tmux new-session -A -s <workspace-agent>`
- `IMPULSE_TERMINAL_TRANSPORT=smux`: use a custom launcher template from `IMPULSE_SMUX_LAUNCHER`
- `IMPULSE_SMUX_LAUNCHER` must contain `{session}` and `{command}` placeholders, for example:
  `smux attach {session} --command {command}`

Transport choice is published through terminal telemetry so the daemon-facing roster can distinguish direct tabs from mux-backed ownership.
